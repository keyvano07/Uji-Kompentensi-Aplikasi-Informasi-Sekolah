            ...
            <table class="min-w-full">
                <thead>
                    <tr class="border-b border-gray-200">
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider w-32">Tipe</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Teks</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider w-40">Dibuat</th>
                        @if(Auth::user()->role === 'admin')
                ...
                <tbody id="infosTableBody" class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors group">
                        ...
                        <td class="px-4 py-4 text-sm font-medium text-gray-900">{{ $info->judul }}</td>
                        <td class="px-4 py-4">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                {{ $info->tipe === 'pengumuman' ? 'bg-blue-100 text-blue-800' : '' }}
                                {{ $info->tipe === 'berita' ? 'bg-green-100 text-green-800' : '' }}
                                {{ $info->tipe === 'info' ? 'bg-gray-100 text-gray-800' : '' }}">
                                {{ ucfirst($info->tipe) }}
                            </span>
                        </td>
                        <td class="px-4 py-4 text-sm text-gray-700">{{ Str::limit($info->text, 50) }}</td>
                        <td class="px-4 py-4 text-xs text-gray-500">{{ $info->created_at->format('d/m/Y, H.i') }}</td>
                        @if(Auth::user()->role === 'admin')
                        <td class="px-4 py-4 text-xs font-medium">
                            <div class="flex gap-3">
                                <button onclick="openEditModal({{ $info->id }}, '{{ addslashes($info->judul) }}', '{{ $info->tipe }}', '{{ addslashes($info->text) }}')" class="text-gray-900 hover:text-bondowoso font-bold">Ubah</button>
    ...
    <div id="addInfoModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50 flex items-center justify-center backdrop-blur-sm">
        <div class="relative p-5 border w-full max-w-md shadow-lg rounded-xl bg-white m-4">
            <div class="mb-5">
                ...
                <p class="text-sm text-gray-500">Masukkan informasi yang akan ditampilkan.</p>
            </div>
            <form id="addInfoForm" action="{{ route('infos.store') }}" method="POST" onsubmit="return handleFormSubmit(this)">
                @csrf
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2">Judul</label>
                    <input type="text" name="judul" class="w-full px-3 py-2 text-gray-700 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" required placeholder="Contoh: Pengumuman Libur">
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2">Tipe</label>
                    <select name="tipe" class="w-full px-3 py-2 text-gray-700 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" required>
                ...
                <div class="mb-5">
                    ...
                    <textarea name="text" rows="3" class="w-full px-3 py-2 text-gray-700 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 resize-none" required placeholder="Deskripsi lengkap..."></textarea>
                </div>
                <div class="flex justify-end gap-3">
                    ...
                    <button type="submit" id="submitAddBtn" class="px-4 py-2 bg-bondowoso text-white rounded-lg hover:bg-green-800 transition-colors font-medium text-sm">Simpan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="editInfoModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50 flex items-center justify-center backdrop-blur-sm">
        <div class="relative p-5 border w-full max-w-md shadow-lg rounded-xl bg-white m-4">
            <div class="mb-5">
                ...
                <p class="text-sm text-gray-500">Edit informasi yang sudah ada.</p>
            </div>
            <form id="editInfoForm" method="POST" onsubmit="return handleFormSubmit(this)">
                @csrf
                @method('PUT')
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2">Judul</label>
                    <input type="text" name="judul" id="editInfoJudul" class="w-full px-3 py-2 text-gray-700 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" required>
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2">Tipe</label>
                    <select name="tipe" id="editInfoTipe" class="w-full px-3 py-2 text-gray-700 border rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500" required>
                        <option value="info">Info</option>
                        <option value="pengumuman">Pengumuman</option>
                        <option value="berita">Berita</option>
                    </select>
